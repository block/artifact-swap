...
class GradleToolingHashingProjectsProviderTest {
    ...
    fun `getProjectHashingInfos filters out root project`() = runTest {
        ...
        val provider = GradleToolingHashingProjectsProvider(
    ...
    fun `getProjectHashingInfos excludes projects with settings gradle`() = runTest {
        ...
        projectThatHasSettingsFileDir.resolve("build.gradle").writeText(
            """
            plugins { id 'groovy-gradle-plugin' }
            """.trimIndent()
        )
        projectThatHasSettingsFileDir.resolve("settings.gradle").writeText(
            """
    ...
    fun `getProjectHashingInfos filters yml and txt files`() = runTest {
        ...
        try {
            ...
            val appFiles = appInfo.filesToHash.map { appInfo.projectDirectory.relativize(it).pathString.replace('\\', '/') }.toList()
    ...
    fun `getProjectHashingInfos handles empty project with no source files`() = runTest {
        ...
        emptyDir.resolve("build.gradle").writeText(
            """
            plugins { id 'java' }
            """.trimIndent()
        )
    ...
    fun `getProjectHashingInfos returns expected projects and files`() = runTest {
        ...
        try {
            ...
            val libFiles = libInfo.filesToHash.map { libInfo.projectDirectory.relativize(it).pathString.replace('\\', '/') }.toList()
            assertEquals(listOf(
                "build.gradle",
    ...
    private fun createAppModule(moduleDir: Path) {
        ...
        moduleDir.resolve("src/main/java/com/example/app/App.java").writeText(
            """
    ...
    private fun createLibModule(moduleDir: Path) {
        ...
        moduleDir.resolve("src/main/kotlin/com/example/lib/Library.kt").writeText(
            """
