...
class ArtifactCheckerServiceTest {
  ...
  private fun createServiceWith(
    existingArtifacts: Set<Pair<String, String>>
  ...
  @Test
  fun `GIVEN all artifacts exist WHEN checking artifacts THEN returns empty missing list`() =
    runTest {
      // Setup: All artifacts exist
      artifactCheckerService =
        createServiceWith(setOf("project1" to "1.0.0", "project2" to "2.0.0"))

      val projectTaskVersions =
        listOf(Triple(":project1", "publish", "1.0.0"), Triple(":project2", "publish", "2.0.0"))

      val result =
        artifactCheckerService.checkArtifacts(
  ...
  fun `GIVEN no artifacts exist WHEN checking artifacts THEN returns all as missing`() = runTest {
    ...
    assertTrue(
      result.missingArtifacts.containsAll(
        listOf(":project1:publish", ":project2:publish", ":project3:publish")
      )
    )
  }

  @Test
  fun `GIVEN empty project list WHEN checking artifacts THEN returns success with zero counts`() =
    runTest {
      artifactCheckerService = createServiceWith(emptySet())
  ...
  fun `GIVEN successful check WHEN logging result THEN sends event to event stream`() = runTest {
    ...
    val projectTaskVersions = listOf(Triple(":project1", "publish", "1.0.0"))
  ...
  fun `GIVEN nested project path WHEN checking artifacts THEN converts path correctly`() = runTest {
    ...
    artifactCheckerService = createServiceWith(setOf("foo_bar_baz" to "1.0.0"))

    val projectTaskVersions = listOf(Triple(":foo:bar:baz", "publish", "1.0.0"))
