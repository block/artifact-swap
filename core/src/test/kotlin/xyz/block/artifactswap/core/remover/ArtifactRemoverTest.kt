...
class ArtifactRemoverTest {
  ...
  @Test
  fun `GIVEN old BOMs beyond retention WHEN executing THEN deletes old BOMs and keeps recent ones`() =
    runTest {
      val recentBoms =
        (0 until ArtifactRemover.NUMBER_OF_BOMS_TO_KEEP).map { index ->
          createTestBom("recent-$index")
        }
      val oldBoms = (0 until 5).map { index -> createTestBom("old-$index") }
  ...
  fun `GIVEN artifacts not in recent BOMs WHEN executing THEN deletes old artifacts`() = runTest {
    val recentBom =
      createTestBom(
        "recent",
        listOf(createTestProject(":app", "1.0.0"), createTestProject(":lib", "2.0.0")),
      )
  ...
  fun `GIVEN deletion failures WHEN executing THEN reports failures correctly`() = runTest {
    ...
    fakeRepository.installedBoms =
      List(ArtifactRemover.NUMBER_OF_BOMS_TO_KEEP + 2) { createTestBom("keep-$it") } +
        listOf(bom1, bom2)
  ...
  @Test
  fun `GIVEN repository measurements WHEN executing THEN captures stats before and after`() =
    runTest {
  ...
  @Test
  fun `GIVEN successful execution WHEN logging result THEN sends event to event stream`() =
    runTest {
      fakeRepository.installedBoms = listOf(createTestBom("test"))
  ...
  @Test
  fun `GIVEN artifacts in multiple recent BOMs WHEN executing THEN keeps all versions in use`() =
    runTest {
      val bom1 = createTestBom("bom-1", listOf(createTestProject(":app", "1.0.0")))
      val bom2 = createTestBom("bom-2", listOf(createTestProject(":app", "1.1.0")))
      fakeRepository.installedBoms = listOf(bom1, bom2)

      fakeRepository.installedProjects =
        listOf(createTestProject(":app", setOf("1.0.0", "1.1.0", "0.9.0")))
  ...
  private fun createTestProject(projectPath: String, version: String): InstalledProject {
  ...
  private fun createTestProject(projectPath: String, versions: Set<String>): InstalledProject {
    return InstalledProject(
      projectPath = projectPath,
      repositoryPath = Path("/fake/path/$projectPath"),
      versions = versions,
