...
class RealArtifactSwapModuleSelector(
  ...
  private val artifactSwapBomHelper: ArtifactSwapBomHelper,
  ...
  private suspend fun determineBomVersion(): Pair<String, Duration> {
    val timedValue = measureTimedValue {
      try {
        coroutineScope {
          val bom = async(ioDispatcher) { artifactSwapBomHelper.findBestBomVersion().getOrThrow() }
          bom.await()
        }
      } catch (e: Exception) {
        ...
        throw RuntimeException("Failed to determine BOM version", e)
      }
    }
