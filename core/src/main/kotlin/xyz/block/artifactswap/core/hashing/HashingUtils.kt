...
/**
 * Hashes a sequence of source file paths using the provided message digest. Returns the hex-encoded
 * hash and the count of files hashed.
 */
fun Sequence<Path>.hashSources(messageDigest: MessageDigest): Pair<String, Int> {
  val pathCount = onEach { path -> messageDigest.update(path.readBytes()) }.count()
