...
class ProjectHashService(
  ...
  suspend fun hashProjects(
    ...
    try {
      ...
      val (projectsToHash, acquireGradleProjectsDuration) =
        measureTimedValue {
      ...
      val (projectHashes, hashDuration) =
        measureTimedValue {
          projectsToHash
            .getHashFlow(applicationDirectory, useBuildLogicVersion, useProjects)
            .onEach { hashingResult ->
              result =
                result.copy(
                  countProjectsHashed =
                    result.countProjectsHashed + if (hashingResult.hash.isNotEmpty()) 1 else 0,
      ...
      result = result.copy(totalDurationMs = Clock.systemUTC().millis() - startTime)
    }
  ...
  private fun List<ProjectHashingInfo>.getHashFlow(
    ...
    return channelFlow {
        ...
        val buildLogicVersion = properties[BUILD_LOGIC_CONSUMER_KEY]?.toString()?.toByteArray()
        logger.debug { "Hashing project source files" }
        this@getHashFlow.filter { useProjects.isEmpty() || it.projectPath in useProjects }
          .forEach { projectHashingInfo ->
            launch(defaultDispatcher) {
              ...
              val (hashAndFilesHashedCount, duration) =
                measureTimedValue { projectHashingInfo.filesToHash.hashSources(messageDigest) }
