...
class TaskFinderService(
  ...
  suspend fun findTasks(
    ...
    try {
      ...
      val (rootProject, getGradleProjectDuration) =
        measureTimedValue {
          withContext(ioDispatcher) {
            projectConnection
              .model(GradleProject::class.java)
      ...
      val (tasks, taskFindingDuration) =
        measureTimedValue { collectAllTasks(rootProject).filter { it.path.endsWith(taskName) } }
      ...
      return TaskFindingResult(
        tasks = tasks,
        serviceResult = result.copy(totalDurationMs = Clock.systemUTC().millis() - startTime),
      )
    } catch (e: Exception) {
  ...
  private fun collectAllTasks(project: GradleProject): List<GradleTask> {
    return buildList {
      ...
      project.children.forEach { child -> addAll(collectAllTasks(child)) }
    }
  }
}
