...
class HashingCommand : AbstractArtifactSwapCommand() {
  ...
  @Mixin private lateinit var hashingOptions: HashingOptions

  @Mixin private lateinit var ciOptions: CiConfigurationOptions
  ...
  override suspend fun executeCommand(application: KoinApplication): Int {
    ...
    val applicationDirectory =
      application.koin.get<Path>(org.koin.core.qualifier.named("directory"))

    // Default hashing file location is ".gradle/sandbagHashes/sandbagHashing.out"
    hashingFile =
      hashingOptions.hashingFile
    ...
    val result =
      projectHashService.hashProjects(
    ...
    logger.info {
      "Project hashing completed: ${result.countProjectsHashed} projects hashed, ${result.countFilesHashed} files"
    }

    // Write results to file
    val hashList =
      result.projectHashes.map { hashResult -> "${hashResult.projectPath}|${hashResult.hash}" }
