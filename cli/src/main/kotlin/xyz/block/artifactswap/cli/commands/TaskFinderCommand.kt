...
class TaskFinderCommand : AbstractArtifactSwapCommand() {
  ...
  @CommandLine.Mixin private lateinit var taskFinderOptions: TaskFinderOptions

  @CommandLine.Mixin private lateinit var ciOptions: CiConfigurationOptions
  ...
  override suspend fun executeCommand(application: KoinApplication) {
    ...
    try {
      ...
      val gradleJvmArgs = application.koin.getOrNull<List<String>>(named("jvmArgs")) ?: emptyList()
      ...
      val updatedResult =
        writeTasksToFiles(taskFindingResult.serviceResult, taskFindingResult.tasks.map { it.path })
  ...
  private suspend fun writeSingleTaskList(
    ...
    return serviceResult.copy(countTasksInLastOutputFile = taskPaths.size)
  }
  ...
  private suspend fun writeShardedTaskList(
    ...
    taskPaths.forEachIndexed { index, taskPath ->
      ...
      tasksInLastOutputFile =
        if (pageCount == taskFinderOptions.pages - 1) {
    ...
    return serviceResult.copy(countTasksInLastOutputFile = tasksInLastOutputFile)
  }
