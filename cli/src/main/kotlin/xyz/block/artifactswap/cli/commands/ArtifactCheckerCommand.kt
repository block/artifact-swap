...
class ArtifactCheckerCommand : AbstractArtifactSwapCommand() {
  ...
  @CommandLine.Mixin private lateinit var artifactCheckerOptions: ArtifactCheckerOptions
  ...
  override suspend fun executeCommand(application: KoinApplication): Int {
    ...
    val (projectTaskVersions, determineProjectsDuration) =
      measureTimedValue {
        try {
          withContext(ioDispatcher) {
            val hashFile =
              artifactCheckerOptions.hashFile.useLines { lines ->
                lines
                  .flatMap { line ->
                    line.split('|').takeIf { it.size == 2 }?.zipWithNext() ?: emptyList()
                  }
                  .toMap()
              }
    ...
    result =
      result.copy(determineProjectsDurationMs = determineProjectsDuration.inWholeMilliseconds)
    ...
    try {
      ...
      result = result.copy(result = ArtifactCheckerResult.FAILED_WRITING_OUTPUT)
    }
