...
class HashingCommandTest {
  ...
  fun `GIVEN valid CLI args WHEN executing THEN command parses args and runs hashing`() = runTest {
    ...
    val command =
      commandLine
        .parseArgs("--hashing-output-file", hashingOutputFile.toString(), "--build_id", "build-123")
        .commandSpec()
        .commandLine()
        .getCommand<HashingCommand>()
  ...
  @Test
  fun `GIVEN multiple projects WHEN executing THEN hashes all projects and writes to file`() =
    runTest {
      ...
      fakeGradleProjectsProvider.projectHashingInfos =
        Result.success(listOf(projectInfo1, projectInfo2))

      val hashingOutputFile = Path(tempDir.absolutePath, "hashes.txt")

      val command =
        commandLine
          .parseArgs("--hashing-output-file", hashingOutputFile.toString())
          .commandSpec()
          .commandLine()
          .getCommand<HashingCommand>()
