...
class BomPublishingCommandTest {
  ...
  @Test
  fun `GIVEN valid CLI args WHEN executing THEN command parses args and runs publisher`() =
    runTest {
      // Create a hash file
      ...
      fakeHashReader.projectHashes =
        Result.success(mapOf("project1" to "version1", "project2" to "version2"))

      // Set up metadata responses
      fakeArtifactoryEndpoints.metadataResponses["project1"] =
        Response.success(
          Metadata(
      ...
      val command =
        commandLine
          ...
          .commandSpec()
          .commandLine()
          .getCommand<BomPublishingCommand>()
  ...
  @Test
  fun `GIVEN published artifacts WHEN executing THEN creates BOM with correct dependencies`() =
    runTest {
      val hashFile = Path(tempDir.absolutePath, "hashes.txt")
      hashFile.writeText("artifact1|v1\nartifact2|v2")

      fakeHashReader.projectHashes = Result.success(mapOf("artifact1" to "v1", "artifact2" to "v2"))
      ...
      val command =
        commandLine
          .parseArgs("--bom-version", "2.0.0", "--hash-file-location", hashFile.toString())
          .commandSpec()
          .commandLine()
          .getCommand<BomPublishingCommand>()
