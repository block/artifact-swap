 ...
 * the core ArtifactCheckerService.
 */
class ArtifactCheckerCommandTest {
  ...
  @Test
  fun `GIVEN valid input files WHEN some artifacts missing THEN outputs missing artifacts to file`() =
    runTest {
      ...
      val fakeArtifactoryService = createFakeArtifactoryServiceForCli(setOf("project1" to "1.0.0"))
      ...
      val command =
        commandLine
          ...
          .commandSpec()
          .commandLine()
          .getCommand<ArtifactCheckerCommand>()
  ...
  fun `GIVEN all artifacts exist WHEN checking THEN outputs empty file`() = runTest {
    ...
    val fakeArtifactoryService =
      createFakeArtifactoryServiceForCli(setOf("project1" to "1.0.0", "project2" to "2.0.0"))
  ...
  @Test
  fun `GIVEN missing input file WHEN executing THEN returns FAILED_NO_INPUT_FILE result`() =
    runTest {
      // Create hash file
      val hashFile = tempDir.toPath().resolve("hash.txt")
      hashFile.writeText(":project1|1.0.0")
  ...
  fun `GIVEN nested project paths WHEN checking THEN converts paths correctly`() = runTest {
    ...
    val fakeArtifactoryService = createFakeArtifactoryServiceForCli(setOf("foo_bar_baz" to "1.0.0"))
...
class FakeArtifactoryEndpointsForCli : xyz.block.artifactswap.core.network.ArtifactoryEndpoints {
  ...
  override suspend fun getPom(
    ...
    artifact: String,
    version: String,
  ): Response<xyz.block.artifactswap.core.maven.Project> {
  ...
  override suspend fun pushMetadata(
    ...
    groupPath: String,
    artifact: String,
    metadata: xyz.block.artifactswap.core.maven.Metadata,
  ): Response<Unit> {
