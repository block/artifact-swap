...
class TaskRunnerCommandTest {
  ...
  @Test
  fun `GIVEN valid task list file WHEN executing THEN command parses args and runs tasks`() =
    runTest {
  ...
  fun `GIVEN empty task list file WHEN executing THEN returns NO_OP result`() = runTest {
    ...
    val command =
      commandLine
        .parseArgs("--dir=${tempDir.absolutePath}", "--task-list-file=${taskListFile}")
        .commandSpec()
        .commandLine()
        .getCommand<TaskRunnerCommand>()
