    ...
    try {
      ...
      val taskDuration = measureTime {
        try {
          ...
          if (
            e is GradleConnectionException &&
      ...
      if (incompleteTasks.isNotEmpty()) {
        ...
        incompleteTasks.forEach { task -> logger.warn { task } }
      }
      ...
      result =
        result.copy(
          ...
          totalDurationMs = Clock.systemUTC().millis() - startTime,
          result =
            if (result.result == TaskRunnerResult.NONE) TaskRunnerResult.SUCCESS else result.result,
        )
