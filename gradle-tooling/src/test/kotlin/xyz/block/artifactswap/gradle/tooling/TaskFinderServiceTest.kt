...
class TaskFinderServiceTest {
  ...
  fun `GIVEN multiple projects WHEN finding tasks THEN returns all matching tasks`() = runTest {
    ...
    createGradleProject(root = tempDir.toPath(), projects = projectNames, taskName = taskName)
  ...
  @Test
  fun `GIVEN projects with subset having task WHEN finding tasks THEN returns only matching tasks`() =
    runTest {
  ...
  @Test
  fun `GIVEN project with no matching tasks WHEN finding tasks THEN returns empty list`() =
    runTest {
  ...
  @Test
  fun `GIVEN successful task finding WHEN logging result THEN sends event to event stream`() =
    runTest {
  ...
  private fun createGradleProject(root: Path, projects: List<String>, taskName: String) {
    ...
    val settingsContent = buildString {
      ...
      projects.forEach { projectName -> appendLine("include '$projectName'") }
    }
  ...
  private fun createModule(rootDir: Path, name: String, contents: String): Path {
    ...
    return rootDir.resolve(pathDir).createDirectories().apply {
      resolve("build.gradle").apply { writeText(contents) }
    }
  }

  private fun createSettingsFile(rootDir: Path, contents: String) {
    rootDir.resolve("settings.gradle").apply { writeText(contents) }
  }
}
